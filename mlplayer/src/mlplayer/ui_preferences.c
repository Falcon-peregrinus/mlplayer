...
create_filepopup_settings(void)
{
    ...
    label_cover_search = gtk_label_new(_("While searching for the album's cover, MLPlayer looks for certain words in the filename. You can specify those words in the lists below, separated using commas."));
...
void * * create_prefs_window (void)
{
    ...
    gtk_window_set_title (GTK_WINDOW (prefswin), _("MLPlayer Preferences"));
    ...
    mlpversionlabel = gtk_label_new ("");
    gtk_box_pack_start (GTK_BOX (hbox4), mlpversionlabel, FALSE, FALSE, 0);
    gtk_label_set_use_markup (GTK_LABEL (mlpversionlabel), TRUE);
    ...
    /* mlplayer version label */

    mlp_version_string = g_strdup_printf
     ("<span size='small'>%s (%s)</span>", "MLPlayer " VERSION, BUILDSTAMP);

    gtk_label_set_markup( GTK_LABEL(mlpversionlabel) , mlp_version_string );
    g_free(mlp_version_string);
    gtk_widget_show_all(vbox);

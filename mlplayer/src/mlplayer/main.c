...
static char * mlp_paths[AUD_PATH_COUNT];
...
static void make_dirs(void)
{
    ...
    make_directory(mlp_paths[AUD_PATH_USER_DIR], mode755);
    make_directory(mlp_paths[AUD_PATH_PLAYLISTS_DIR], mode755);
}
...
static void relocate_paths (void)
{
    ...
    mlp_paths[AUD_PATH_BIN_DIR] = g_strdup (HARDCODE_BINDIR);
    mlp_paths[AUD_PATH_DATA_DIR] = g_strdup (HARDCODE_DATADIR);
    mlp_paths[AUD_PATH_PLUGIN_DIR] = g_strdup (HARDCODE_PLUGINDIR);
    mlp_paths[AUD_PATH_LOCALE_DIR] = g_strdup (HARDCODE_LOCALEDIR);
    mlp_paths[AUD_PATH_DESKTOP_FILE] = g_strdup (HARDCODE_DESKTOPFILE);
    mlp_paths[AUD_PATH_ICON_FILE] = g_strdup (HARDCODE_ICONFILE);
    normalize_path (mlp_paths[AUD_PATH_BIN_DIR]);
    normalize_path (mlp_paths[AUD_PATH_DATA_DIR]);
    normalize_path (mlp_paths[AUD_PATH_PLUGIN_DIR]);
    normalize_path (mlp_paths[AUD_PATH_LOCALE_DIR]);
    normalize_path (mlp_paths[AUD_PATH_DESKTOP_FILE]);
    normalize_path (mlp_paths[AUD_PATH_ICON_FILE]);

    /* Compare the compile-time path to the executable and the actual path to
     * see if we have been moved. */
    char * old = g_strdup (mlp_paths[AUD_PATH_BIN_DIR]);
...
ERR:
    ...
    /* Do the replacements. */
    relocate_path (& mlp_paths[AUD_PATH_BIN_DIR], old, new);
    relocate_path (& mlp_paths[AUD_PATH_DATA_DIR], old, new);
    relocate_path (& mlp_paths[AUD_PATH_PLUGIN_DIR], old, new);
    relocate_path (& mlp_paths[AUD_PATH_LOCALE_DIR], old, new);
    relocate_path (& mlp_paths[AUD_PATH_DESKTOP_FILE], old, new);
    relocate_path (& mlp_paths[AUD_PATH_ICON_FILE], old, new);
...
static void init_paths (void)
{
    ...
    mlp_paths[AUD_PATH_USER_DIR] = g_build_filename(xdg_config_home, "mlplayer", NULL);
    mlp_paths[AUD_PATH_USER_PLUGIN_DIR] = g_build_filename(xdg_data_home, "mlplayer", "Plugins", NULL);
    mlp_paths[AUD_PATH_PLAYLISTS_DIR] = g_build_filename(mlp_paths[AUD_PATH_USER_DIR], "playlists", NULL);
    mlp_paths[AUD_PATH_GTKRC_FILE] = g_build_filename(mlp_paths[AUD_PATH_USER_DIR], "gtkrc", NULL);
...
{
    char * path = g_strdup_printf ("%s" G_DIR_SEPARATOR_S "lock", mlp_paths[AUD_PATH_USER_DIR]);
...
{
#ifdef USE_DBUS
    DBusGProxy * session = mlplayer_get_dbus_proxy ();

    if (session && mlplayer_remote_is_running (session))
    {
        ...
        if (filenames)
        {
            ...
            if (options.enqueue_to_temp)
                mlplayer_remote_playlist_open_list_to_temp (session, list);
            else if (options.enqueue)
                mlplayer_remote_playlist_add (session, list);
            else
                mlplayer_remote_playlist_open_list (session, list);
        ...
        if (options.play)
            mlplayer_remote_play (session);
        if (options.pause)
            mlplayer_remote_pause (session);
        if (options.play_pause)
            mlplayer_remote_play_pause (session);
        if (options.stop)
            mlplayer_remote_stop (session);
        if (options.rew)
            mlplayer_remote_playlist_prev (session);
        if (options.fwd)
            mlplayer_remote_playlist_next (session);
        if (options.show_jump_box)
            mlplayer_remote_show_jtf_box (session);
        if (options.mainwin)
            mlplayer_remote_main_win_toggle (session, TRUE);

        exit (EXIT_SUCCESS);
    }
#endif

    fprintf (stderr, "WARNING: MLPlayer seems to be already running but is not responding.\n");
}
...
static void init_one (void)
{
    ...
    bindtextdomain (PACKAGE, mlp_paths[AUD_PATH_LOCALE_DIR]);
    bind_textdomain_codeset (PACKAGE, "UTF-8");
    bindtextdomain (PACKAGE "-plugins", mlp_paths[AUD_PATH_LOCALE_DIR]);
    bind_textdomain_codeset (PACKAGE "-plugins", "UTF-8");
    textdomain (PACKAGE);

#ifdef USE_EGGSM
    egg_sm_client_set_mode (EGG_SM_CLIENT_MODE_NORMAL);
    egg_set_desktop_file (mlp_paths[AUD_PATH_DESKTOP_FILE]);
#endif
}
...
static void init_two (int * p_argc, char * * * p_argv)
{
    ...
    {
        g_thread_init (NULL);
        gtk_rc_add_default_file (mlp_paths[AUD_PATH_GTKRC_FILE]);

...
static void load_playlists_real (void)
{
    ...
    {
        char * path = g_strdup_printf ("%s/%s.mlppl", get_path (AUD_PATH_PLAYLISTS_DIR), order[i]);
...
static void save_playlists_real (void)
{
    ...
    for (int i = 0; i < lists; i ++)
    {
        ...
        {
            char * path = g_strdup_printf ("%s/%d.mlppl", folder, id);
            char * uri = filename_to_uri (path);
        ...
        g_hash_table_insert (saved, g_strdup_printf ("%d.mlppl", id), NULL);
    }
    ...
    {
        if (! g_str_has_suffix (entry->d_name, ".mlppl")

...
/* Sets the filename associated with a playlist.  (MLPlayer currently makes no
 * use of the filename.) */
 ...
 * MLPlayer will attempt to read metadata from the song file.  The caller gives
 * up one reference count to <tuple>.  If <play> is nonzero, MLPlayer will
 ...
 * (see libmlpcore/index.h), must be pooled with str_get(); the caller gives up

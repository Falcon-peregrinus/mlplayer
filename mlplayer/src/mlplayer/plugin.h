/*
 * plugin.h
 * Copyright 2005-2010 MLPlayer Development Team
 *
...
/* "Magic" bytes identifying an MLPlayer plugin header. */
 ...
 * some parts of the MLPlayer API are *not* thread-safe and therefore cannot be
 ...
 *
 * Since 3.2, MLPlayer implements a basic messaging system between plugins.
 * Messages are sent using mlp_plugin_send_message() and received through the
 ...
 *
 * For the time being, mlp_plugin_send_message() should only be called from the
...
struct _OutputPlugin
{
     ...
     * enumeration values defined in libmlpcore/audio.h.  Returns nonzero on
     * success. */
...
struct _InputPlugin
{
     ...
     * determined that this plugin can handle the file, MLPlayer opens the file
     * and calls probe_for_tuple().  probe_for_tuple() sees that there are 3
     * songs in the file and sets FIELD_SUBSONG_NUM to 3.
     * 2. For each song in the file, MLPlayer opens the file and calls
     ...
     * 3. When one of the songs is played, MLPlayer opens the file and calls
     ...
     * NULL.  MLPlayer takes over one reference to the tuple returned. */
     ...
     * GTK.  MLPlayer will free the data when it is no longer needed. */
    ...
    /* Advanced, for plugins that do not use MLPlayer's output system.  Use at
     * your own risk. */

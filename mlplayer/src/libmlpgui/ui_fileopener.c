 ...
 bool_t play)
{
    ...
    if (play)
        mlp_drct_pl_open_list (list);
    else
        mlp_drct_pl_add_list (list, -1);

    char * path = gtk_file_chooser_get_current_folder (browser);
    if (path)
    {
        mlp_set_string ("mlpgui", "filesel_path", path);
        g_free (path);
    }
}
...
{
    mlp_set_bool ("mlpgui", (const char *) option, gtk_toggle_button_get_active (toggle));
}
...
run_filebrowser_gtk2style(bool_t play_button, bool_t show)
{
    ...
    char * path = mlp_get_string ("mlpgui", "filesel_path");
    if (path[0])
 ...
 */
EXPORT void
mlpgui_run_filebrowser(bool_t play_button)
{
    run_filebrowser_gtk2style(play_button, TRUE);
}

EXPORT void
mlpgui_hide_filebrowser(void)
{

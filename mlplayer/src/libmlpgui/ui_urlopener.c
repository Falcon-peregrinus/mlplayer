...
{
    mlp_history_add (gtk_entry_get_text (entry));
}
...
static GtkWidget * urlopener_add_url_dialog_new (GCallback func, bool_t open)
{
    ...
    const char * path;
    for (int i = 0; (path = mlp_history_get (i)); i ++)
                      ...
                      GtkWidget * entry)
{
    ...
    if (text != NULL && * text)
        mlp_drct_pl_open (text);
}

EXPORT void mlpgui_show_add_url_window (bool_t open)
{
    ...
    if (!url_window) {
        ...
        mlpgui_destroy_on_escape (url_window);

...
{
    GdkPixbuf * p = mlpgui_pixbuf_for_entry (list, entry);
    g_return_if_fail (p);

    mlpgui_pixbuf_scale_within (& p, IMAGE_SIZE);
...
static void infowin_update_tuple (void * unused)
{
    ...
    if (mlp_file_write_tuple (current_file, current_decoder, tuple))
    {
...
static void create_infowin (void)
{
    ...
    mlpgui_hide_on_delete (infowin);
    mlpgui_hide_on_escape (infowin);
...
EXPORT void mlpgui_infowin_show (int playlist, int entry)
{
    char * filename = mlp_playlist_entry_get_filename (playlist, entry);
    g_return_if_fail (filename != NULL);

    PluginHandle * decoder = mlp_playlist_entry_get_decoder (playlist, entry,
     FALSE);
    if (decoder == NULL)
        goto FREE;

    if (mlp_custom_infowin (filename, decoder))
        goto FREE;

    Tuple * tuple = mlp_playlist_entry_get_tuple (playlist, entry, FALSE);

    if (tuple == NULL)
    {
        char * message = g_strdup_printf (_("No info available for %s.\n"),
         filename);
        mlp_interface_show_error (message);
        g_free (message);
        goto FREE;
    }

    infowin_show (playlist, entry, filename, tuple, decoder,
     mlp_file_can_write_tuple (filename, decoder));
    tuple_unref (tuple);
...
EXPORT void mlpgui_infowin_show_current (void)
{
    int playlist = mlp_playlist_get_playing ();
    int position;

    if (playlist == -1)
        playlist = mlp_playlist_get_active ();

    position = mlp_playlist_get_position (playlist);

    if (position == -1)
        return;

    mlpgui_infowin_show (playlist, position);
}

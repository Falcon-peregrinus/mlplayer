...
# Support mlptool as a fallback but don't depend on it
...
def addTrack(path, play=False):
	try:
		...
		try:
			if subprocess.call(['mlptool','playlist-addurl',file_url]):
				print "ERROR: mlptool fallback returned an error for: %s" % file_url
			elif play:
				os.system('mlptool playlist-jump `mlptool playlist-length`; mlptool playback-play')
		except OSError:
			print "ERROR: Unable to call mlptool as a fallback for: %s" % file_url

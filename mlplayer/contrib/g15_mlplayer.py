...
MLPlayer Media Player Monitor applet for the Logitech G15 Gaming Keyboard's LCD (via g15composer)

Requires MLPlayer 1.4 or newer. (Uses the D-Bus API)
...
message_pipe = os.path.join(os.environ["HOME"], ".g15_mlplayer_pipe")

def get_mlp():
	try: return session_bus.get_object('org.atheme.mlplayer', '/org/atheme/mlplayer')
...
try:
	...
	while True:
		...
		if mlp:
			...
			lengthSecs = mlp.SongLength(pos)
			length = (lengthSecs > 0) and ("%d:%02d" % (lengthSecs / 60, lengthSecs % 60)) or "stream"

			songTitle = mlp.SongTitle(pos).encode("latin1", "replace")
			...
			msg_handle.write('PC 0\nTL "D-Bus Exception:" "Can\'t find MLPlayer"\n"')
finally:
	msg_handle.write("SC\n")
	msg_handle.close()

...
ifdef([mlp_plugin_dirs_defined],[],
[    pluginsubs="\\\"Plugins\\\""
])dnl
    Name[]_PLUGIN_DIR="Plugins"
else
    ifdef([mlp_def_plugin_dirs_defined],
    [pluginsubs="$pluginsubs,\\\"$1\\\""],
    [pluginsubs="\\\"$1\\\""])
    Name[]_PLUGIN_DIR="$1"
fi
AC_SUBST(Name[]_PLUGIN_DIR)dnl
define([mlp_def_plugin_dirs_defined],[1])dnl
])dnl
...
ifdef([mlp_get_plugin_dirs_defined],
[pluginsubs="$pluginsubs,\\\"$1\\\""],
[pluginsubs="\\\"$1\\\""])
BigName[]_PLUGIN_DIR=`pkg-config mlplayer --variable=[]Name[]`
AC_SUBST(BigName[]_PLUGIN_DIR)dnl
define([mlp_get_plugin_dirs_defined],[1])dnl
])dnl
...
res_desc_[]Name="$3"; ifdef([mlp_def_plugin_$2], [$2[]_PLUGINS="${$2[]_PLUGINS} $1"], [$2[]_PLUGINS="$1"])dnl
define([mlp_def_plugin_$2],[1])dnl
])

...
{
    GList *up_list=NULL;
    gint current_play = mlp_playlist_get_active();
    gint i = (mlp_playlist_entry_count(current_play) - 1);

    for (; i >= 0; i--)
    {
        if (mlp_playlist_entry_get_selected(current_play, i))
        {
            Tuple * tuple = mlp_playlist_entry_get_tuple (current_play, i, FALSE);
            mlp_playlist_entry_set_selected(current_play, i, FALSE);
...
static gboolean mtp_init (void)
{
    ...
    mlp_plugin_menu_add (AUD_MENU_MAIN, mtp_press, _("Upload to MTP Device"), NULL);
    mlp_plugin_menu_add (AUD_MENU_MAIN, free_device, _("Disconnect MTP Device"), NULL);
...
void mtp_cleanup(void)
{
    if (plugin_active)
    {
        mlp_plugin_menu_remove (AUD_MENU_MAIN, mtp_press);
        mlp_plugin_menu_remove (AUD_MENU_MAIN, free_device);

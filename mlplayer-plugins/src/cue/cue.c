 ...
 gchar * * title, Index * filenames, Index * tuples)
{
    ...
    * title = NULL;
    ...
    gchar * filename = mlp_construct_uri (track_filename,
     cue_filename);
    ...
    for (gint track = 1; track <= tracks; track ++)
    {
        ...
        if (base_tuple == NULL && ! base_tuple_scanned)
        {
            ...
            PluginHandle * decoder = mlp_file_find_decoder (filename, FALSE);
            if (decoder != NULL)
                base_tuple = mlp_file_read_tuple (filename, decoder);
        }

        Track * next = (track + 1 <= tracks) ? cd_get_track (cd, track + 1) :
         NULL;
        gchar * next_filename = (next != NULL) ? mlp_construct_uri

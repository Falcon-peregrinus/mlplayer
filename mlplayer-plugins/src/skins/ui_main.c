...
{
    if (title != NULL)
        return g_strdup_printf(_("%s - MLPlayer"), title);
    else
        return g_strdup(_("MLPlayer"));
}
 ...
 unused)
{
    switch (event->direction) {
        ...
        case GDK_SCROLL_LEFT:
            mlp_drct_seek (mlp_drct_get_time () - 5000);
            break;
 ...
 gboolean rewind)
{
    ...
    if (! mlp_drct_get_playing () || time_diff (seek_time, time_now ()) <
     SEEK_THRESHOLD)
    {
        if (rewind)
            mlp_drct_pl_prev ();
...
{
    if (ab_position_a != -1)
        mlp_drct_seek(ab_position_a / 1000);
...
static void mainwin_spos_release_cb (void)
{
    ...
    gint pos = hslider_get_pos (mainwin_sposition);
    mlp_drct_seek (mlp_drct_get_length () * (pos - 1) / 12);
}

static void mainwin_position_motion_cb (void)
{
    gint length = mlp_drct_get_length () / 1000;
...
mainwin_adjust_volume_motion(gint v)
{
    ...
    mlp_drct_set_volume_main (v);
    mlp_drct_set_volume_balance (balance);
}
...
mainwin_setup_menus(void)
{
    ...
    check_set (toggleaction_group_others, "stop after current song",
     mlp_get_bool (NULL, "stop_after_current_song"));

    /* Playback menu */

    check_set (toggleaction_group_others, "playback repeat", mlp_get_bool (NULL, "repeat"));
    check_set (toggleaction_group_others, "playback shuffle", mlp_get_bool (NULL, "shuffle"));
    check_set (toggleaction_group_others, "playback no playlist advance",
     mlp_get_bool (NULL, "no_playlist_advance"));
...
mainwin_create_widgets(void)
{
    ...
    button_on_release (mainwin_close, (ButtonCB) mlp_drct_quit);
    ...
    button_on_release (mainwin_pause, (ButtonCB) mlp_drct_pause);
    ...
    button_on_release (mainwin_stop, (ButtonCB) mlp_drct_stop);
    ...
    button_on_release (mainwin_about, (ButtonCB) mlpgui_show_about_window);
    ...
    button_on_release (mainwin_shaded_close, (ButtonCB) mlp_drct_quit);

    mainwin_srew = button_new_small (8, 7);
    window_put_widget (mainwin, TRUE, mainwin_srew, 169, 4);
    button_on_release (mainwin_srew, (ButtonCB) mlp_drct_pl_prev);
    ...
    button_on_release (mainwin_spause, (ButtonCB) mlp_drct_pause);

    mainwin_sstop = button_new_small (9, 7);
    window_put_widget (mainwin, TRUE, mainwin_sstop, 197, 4);
    button_on_release (mainwin_sstop, (ButtonCB) mlp_drct_stop);

    mainwin_sfwd = button_new_small (8, 7);
    window_put_widget (mainwin, TRUE, mainwin_sfwd, 206, 4);
    button_on_release (mainwin_sfwd, (ButtonCB) mlp_drct_pl_next);
...
mainwin_create_window(void)
{
    ...
    gtk_window_set_title(GTK_WINDOW(mainwin), _("MLPlayer"));
...
static void mainwin_update_volume (void)
{
    ...
    mlp_drct_get_volume_balance (& balance);
...
void mainwin_update_song_info (void)
{
    ...
    {
        time = mlp_drct_get_time ();
        length = mlp_drct_get_length ();
    }
...
{
    mlp_set_bool (NULL, "no_playlist_advance", gtk_toggle_action_get_active (action));
...
{
    mlp_set_bool (NULL, "repeat", gtk_toggle_action_get_active (action));
    button_set_active (mainwin_repeat, gtk_toggle_action_get_active (action));
}

void action_playback_shuffle (GtkToggleAction * action)
{
    mlp_set_bool (NULL, "shuffle", gtk_toggle_action_get_active (action));
...
void action_stop_after_current_song (GtkToggleAction * action)
{
    ...
    if (active != mlp_get_bool (NULL, "stop_after_current_song"))
    {
        if (active)
        ...
        mlp_set_bool (NULL, "stop_after_current_song", active);
    }
}
...
{
    mlpgui_run_filebrowser(TRUE); /* TRUE = PLAY_BUTTON */
}

void action_play_location (void)
{
    mlpgui_show_add_url_window (TRUE);
}

void action_ab_set (void)
{
    if (mlp_drct_get_length () > 0)
    {
        if (ab_position_a == -1)
        {
            ab_position_a = mlp_drct_get_time();
            ab_position_b = -1;
            mainwin_lock_info_text("LOOP-POINT A POSITION SET.");
        }
        else if (ab_position_b == -1)
        {
            int time = mlp_drct_get_time();

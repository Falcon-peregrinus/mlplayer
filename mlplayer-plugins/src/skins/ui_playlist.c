...
#include "images/mlplayer_playlist.xpm"
...
static void playlistwin_update_info (void)
{
    ...
    total = mlp_playlist_get_total_length (active_playlist) / 1000;
    selection = mlp_playlist_get_selected_length (active_playlist) / 1000;
...
{
    gint playlist = mlp_playlist_get_active ();
    gint entry = mlp_playlist_get_position (playlist);
    gchar scratch[512];
    ...
    if (entry > -1)
    {
        gint length = mlp_playlist_entry_get_length (playlist, entry, TRUE);

        if (mlp_get_bool (NULL, "show_numbers_in_pl"))
            snprintf (scratch, sizeof scratch, "%d. ", 1 + entry);

        gchar * title = mlp_playlist_entry_get_title (playlist, entry, TRUE);
...
{
    mlp_playlist_select_all (active_playlist, 1);
}

static void
playlistwin_select_none(void)
{
    mlp_playlist_select_all (active_playlist, 0);
}

static void copy_selected_to_new (gint playlist)
{
    gint entries = mlp_playlist_entry_count (playlist);
    gint new = mlp_playlist_count ();
    Index * filenames = index_new ();
    Index * tuples = index_new ();
    gint entry;

    mlp_playlist_insert (new);

    for (entry = 0; entry < entries; entry ++)
    {
        if (mlp_playlist_entry_get_selected (playlist, entry))
        {
            index_append (filenames, mlp_playlist_entry_get_filename (playlist, entry));
            index_append (tuples, mlp_playlist_entry_get_tuple (playlist, entry, TRUE));
        }
    }

    mlp_playlist_entry_insert_batch (new, 0, filenames, tuples, FALSE);
    mlp_playlist_set_active (new);
}
...
playlistwin_select_search(void)
{
    ...
    switch(result)
    {
      case GTK_RESPONSE_ACCEPT:
      {
         ...
         mlp_playlist_select_by_patterns (active_playlist, tuple);
         tuple_unref (tuple);
         ...
         else
         {
             ...
             gint entries = mlp_playlist_entry_count (active_playlist);
             gint count;

             for (count = 0; count < entries; count ++)
             {
                 if (mlp_playlist_entry_get_selected (active_playlist, count))
                 {
             ...
             if (gtk_toggle_button_get_active ((GtkToggleButton *)
              searchdlg_checkbt_autoenqueue))
                 mlp_playlist_queue_insert_selected (active_playlist, -1);
         }
...
static void playlistwin_inverse_selection (void)
{
    ...
    for (entry = 0; entry < entries; entry ++)
        mlp_playlist_entry_set_selected (active_playlist, entry,
         ! mlp_playlist_entry_get_selected (active_playlist, entry));
}
...
playlistwin_fileinfo(void)
{
    ...
    ui_skinned_playlist_row_info (playlistwin_list, & rows, & first, & focused);
    mlpgui_infowin_show (active_playlist, focused);
}
...
{
    mlpgui_urilist_insert (active_playlist, drop_position, (const gchar *)
...
playlistwin_create_widgets(void)
{
    ...
    button_on_release (playlistwin_srew, (ButtonCB) mlp_drct_pl_prev);
...
playlistwin_create_window(void)
{
    ...
    gtk_window_set_title(GTK_WINDOW(playlistwin), _("MLPlayer Playlist Editor"));

    gtk_window_set_transient_for(GTK_WINDOW(playlistwin),
                                 GTK_WINDOW(mainwin));
    gtk_window_set_skip_taskbar_hint(GTK_WINDOW(playlistwin), TRUE);

    icon = gdk_pixbuf_new_from_xpm_data((const gchar **) mlplayer_playlist_icon);
...
{
    gint playlists = mlp_playlist_count ();

    g_free (active_title);

    if (playlists > 1)
    {
        gchar * title = mlp_playlist_get_title (active_playlist);
...
static void update_cb (void * unused, void * another)
{
    ...
    active_playlist = mlp_playlist_get_active ();
    active_length = mlp_playlist_entry_count (active_playlist);
    get_title ();
    ...
    if (song_changed)
    {
        ui_skinned_playlist_set_focused (playlistwin_list,
         mlp_playlist_get_position (active_playlist));
        song_changed = FALSE;
    }
...
static void follow_cb (void * data, void * another)
{
    ...
    mlp_playlist_select_all (list, FALSE);

    gint row = mlp_playlist_get_position (list);
    if (row >= 0)
        mlp_playlist_entry_set_selected (list, row, TRUE);
...
playlistwin_create(void)
{
    ...
    mlp_playlist_select_all (active_playlist, FALSE);

    gint row = mlp_playlist_get_position (active_playlist);
    if (row >= 0)
        mlp_playlist_entry_set_selected (active_playlist, row, TRUE);
...
void action_queue_toggle (void)
{
    ...
    ui_skinned_playlist_row_info (playlistwin_list, & rows, & first, & focused);
    at = (focused == -1) ? -1 : mlp_playlist_queue_find_entry (active_playlist,
     focused);

    if (at == -1)
        mlp_playlist_queue_insert_selected (active_playlist, -1);
    else
        mlp_playlist_queue_delete (active_playlist, at, 1);
}

void action_playlist_sort_by_track_number (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_TRACK);
}

void action_playlist_sort_by_title (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_TITLE);
}

void action_playlist_sort_by_album (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_ALBUM);
}

void action_playlist_sort_by_artist (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_ARTIST);
}

void action_playlist_sort_by_full_path (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_PATH);
}

void action_playlist_sort_by_date (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_DATE);
}

void action_playlist_sort_by_filename (void)
{
    mlp_playlist_sort_by_scheme (active_playlist, PLAYLIST_SORT_FILENAME);
}

void action_playlist_sort_selected_by_track_number (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_TRACK);
}

void action_playlist_sort_selected_by_title (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_TITLE);
}

void action_playlist_sort_selected_by_album (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_ALBUM);
}

void action_playlist_sort_selected_by_artist (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_ARTIST);
}

void action_playlist_sort_selected_by_full_path (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_PATH);
}

void action_playlist_sort_selected_by_date (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_DATE);
}

void action_playlist_sort_selected_by_filename (void)
{
    mlp_playlist_sort_selected_by_scheme (active_playlist, PLAYLIST_SORT_FILENAME);
}

void action_playlist_randomize_list (void)
{
    mlp_playlist_randomize (active_playlist);
}

void action_playlist_reverse_list (void)
{
    mlp_playlist_reverse (active_playlist);
}

void action_playlist_clear_queue (void)
{
    mlp_playlist_queue_delete (active_playlist, 0, mlp_playlist_queue_count
     (active_playlist));
}

void action_playlist_remove_unavailable (void)
{
    mlp_playlist_remove_failed (active_playlist);
}

void action_playlist_remove_dupes_by_title (void)
{
    mlp_playlist_remove_duplicates_by_scheme (active_playlist,
     PLAYLIST_SORT_TITLE);
}
...
{
    mlp_playlist_entry_delete (active_playlist, 0, mlp_playlist_entry_count
     (active_playlist));
}

void action_playlist_remove_selected (void)
{
    mlp_playlist_delete_selected (active_playlist);
}
...
void action_playlist_remove_unselected (void)
{
    ...
    mlp_playlist_delete_selected (active_playlist);
    mlp_playlist_select_all (active_playlist, TRUE);
}

void action_playlist_copy (void)
{
    GtkClipboard * clip = gtk_clipboard_get (GDK_SELECTION_CLIPBOARD);
    gchar * list = mlpgui_urilist_create_from_selected (active_playlist);
...
void action_playlist_paste (void)
{
    ...
    ui_skinned_playlist_row_info (playlistwin_list, & rows, & first, & focused);
    mlpgui_urilist_insert (active_playlist, focused, list);
    g_free (list);
}
...
{
    mlpgui_run_filebrowser(FALSE); /* FALSE = NO_PLAY_BUTTON */
}

void
action_playlist_add_url(void)
{
    mlpgui_show_add_url_window (FALSE);
}

void action_playlist_new (void)
{
    gint playlist = mlp_playlist_count ();

    mlp_playlist_insert (playlist);
    mlp_playlist_set_active (playlist);
}

void action_playlist_prev (void)
{
    if (active_playlist > 0)
        mlp_playlist_set_active (active_playlist - 1);
}

void action_playlist_next (void)
{
    if (active_playlist + 1 < mlp_playlist_count ())
        mlp_playlist_set_active (active_playlist + 1);
}

void action_playlist_delete (void)
{
    mlpgui_confirm_playlist_delete (active_playlist);
}

void
action_playlist_refresh_list(void)
{
    mlp_playlist_rescan (active_playlist);
}

 ...
 *  Copyright (C) 2005-2009  MLPlayer development team
 *
 *  Based on BMP:
...
#ifdef HAVE_MKDTEMP
    tmpdir = g_build_filename(g_get_tmp_dir(), "mlplayer.XXXXXXXX", NULL);
    if (!mkdtemp(tmpdir))
    {
        g_free(tmpdir);
...
#else
    tmpdir = g_strdup_printf("%s/mlplayer.%ld", g_get_tmp_dir(), (long)rand());

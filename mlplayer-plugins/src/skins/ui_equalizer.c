...
#include "images/mlplayer_eq.xpm"
...
static void eq_on_cb (GtkWidget * button, GdkEventButton * event)
 {mlp_set_bool (NULL, "equalizer_active", button_get_active (button)); }
static void eq_auto_cb (GtkWidget * button, GdkEventButton * event)
 {mlp_set_bool (NULL, "equalizer_autoload", button_get_active (equalizerwin_auto)); }

static void update_from_config (void * unused1, void * unused2)
{
    button_set_active (equalizerwin_on, mlp_get_bool (NULL, "equalizer_active"));
    eq_slider_set_val (equalizerwin_preamp, mlp_get_double (NULL, "equalizer_preamp"));

    gdouble bands[AUD_EQUALIZER_NBANDS];
    mlp_eq_get_bands (bands);
...
static GtkWidget * get_eq_effects_menu (void)
{
    ...
    if (menu == NULL)
        menu = mlpgui_create_effects_menu ();
...
equalizerwin_create_widgets(void)
{
    ...
    button_set_active (equalizerwin_auto, mlp_get_bool (NULL, "equalizer_autoload"));
...
equalizerwin_create_window(void)
{
    ...
    gtk_window_set_title(GTK_WINDOW(equalizerwin), _("MLPlayer Equalizer"));
    ...
    icon = gdk_pixbuf_new_from_xpm_data((const gchar **) mlplayer_eq_icon);
...
{
    equalizer_presets = mlp_equalizer_read_presets("eq.preset");
    equalizer_auto_presets = mlp_equalizer_read_presets("eq.auto_preset");
    ...
    int playlist = mlp_playlist_get_playing ();
 ...
 const char * filename)
{
    ...
    mlp_equalizer_write_preset_file(list, filename);
}
...
{
    Index * presets;
    if ((presets = mlp_import_winamp_eqf(file)) == NULL)
        return;
...
static gboolean equalizerwin_read_mlp_preset (const gchar * file)
{
    EqualizerPreset * preset = mlp_load_preset_file (file);
...
import_winamp_file(const gchar * filename)
{
    ...
    Index * list = mlp_import_winamp_eqf (file);
    if (! list)
        goto CLOSE;
    ...
    mlp_equalizer_write_preset_file(equalizer_presets, "eq.preset");
...
action_equ_load_preset_file(void)
{
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        EqualizerPreset *preset = mlp_load_preset_file(file_uri);
        equalizerwin_apply_preset(preset);
        equalizer_preset_free(preset);
        g_free(file_uri);
    }
...
action_equ_save_auto_preset(void)
{
    ...
    char * name = mlp_drct_get_filename ();
...
action_equ_save_preset_file(void)
{
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        for (i = 0; i < AUD_EQUALIZER_NBANDS; i++)
            preset->bands[i] = equalizerwin_get_band(i);
        mlp_save_preset_file(preset, file_uri);
        equalizer_preset_free(preset);
        g_free(file_uri);
    }

    char * songname = mlp_drct_get_filename ();

    if (songname != NULL)
    {
        gchar * ext = mlp_get_string (NULL, "eqpreset_extension");
...
static void load_auto_preset (const gchar * filename)
{
    ...
    if (ext[0])
    {
        ...
        gboolean success = equalizerwin_read_mlp_preset (eq_file);
        g_free (eq_file);
    ...
    gchar * deffile = mlp_get_string (NULL, "eqpreset_default_file");
    if (deffile[0])
    {
...
static void position_cb (void * data, void * user_data)
{
    ...
    if (! mlp_get_bool (NULL, "equalizer_autoload") || playlist !=
     mlp_playlist_get_playing () || position == -1)
        return;

    gchar * filename = mlp_playlist_entry_get_filename (playlist, position);

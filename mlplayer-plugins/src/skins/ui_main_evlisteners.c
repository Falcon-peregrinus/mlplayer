/*
 * MLPlayer
 * Copyright (c) 2006-2007 MLPlayer development team.
 *
...
static void info_change (void)
{
    ...
    if (mlp_drct_get_ready ())
        mlp_drct_get_info (& bitrate, & samplerate, & channels);
...
void ui_main_evlistener_playback_begin (void * hook_data, void * user_data)
{
    ...
    if (mlp_drct_get_ready () && mlp_drct_get_length () > 0)
    {
...
void start_stop_visual (gboolean exiting)
{
    ...
    if (config.player_visible && config.vis_type != VIS_OFF && ! exiting)
    {
        if (! started)
        {
            mlp_vis_func_add (AUD_VIS_TYPE_CLEAR, (VisFunc) vis_clear_cb);
            mlp_vis_func_add (AUD_VIS_TYPE_MONO_PCM, (VisFunc) render_mono_pcm);
            mlp_vis_func_add (AUD_VIS_TYPE_MULTI_PCM, (VisFunc) render_multi_pcm);
            mlp_vis_func_add (AUD_VIS_TYPE_FREQ, (VisFunc) render_freq);
            started = 1;
        }
    }
    else
    {
        if (started)
        {
            mlp_vis_func_remove ((VisFunc) vis_clear_cb);
            mlp_vis_func_remove ((VisFunc) render_mono_pcm);
            mlp_vis_func_remove ((VisFunc) render_multi_pcm);
            mlp_vis_func_remove ((VisFunc) render_freq);
            started = 0;
        }
    }
}

...
DRAW_FUNC_BEGIN (playlist_draw)
    ...
    gint active_entry = mlp_playlist_get_position (active_playlist);
    gint left = 3, right = 3;
    PangoLayout * layout;
    gint width;
    ...
    for (gint i = data->first; i < data->first + data->rows && i <
     active_length; i ++)
    {
        if (! mlp_playlist_entry_get_selected (active_playlist, i))
            continue;
    ...
    for (gint i = data->first; i < data->first + data->rows && i <
     active_length; i ++)
    {
        gint len = mlp_playlist_entry_get_length (active_playlist, i, TRUE);
        gchar buf[16];
    ...
    if (mlp_playlist_queue_count (active_playlist))
    {
        width = 0;
        ...
        for (gint i = data->first; i < data->first + data->rows && i <
         active_length; i ++)
        {
            gint pos = mlp_playlist_queue_find_entry (active_playlist, i);
            if (pos < 0)
                continue;
    ...
    for (gint i = data->first; i < data->first + data->rows && i <
     active_length; i ++)
    {
        gchar * title = mlp_playlist_entry_get_title (active_playlist, i, TRUE);
...
static void select_single (PlaylistData * data, gboolean relative, gint position)
{
    ...
    if (data->focused != -1)
        mlp_playlist_entry_set_selected (active_playlist, data->focused, FALSE);

    if (mlp_playlist_selected_count (active_playlist) > 0)
        mlp_playlist_select_all (active_playlist, FALSE);

    mlp_playlist_entry_set_selected (active_playlist, position, TRUE);
...
static void select_extend (PlaylistData * data, gboolean relative, gint position)
{
    ...
    for (; count != position; count += sign)
        mlp_playlist_entry_set_selected (active_playlist, count,
         ! mlp_playlist_entry_get_selected (active_playlist, count + sign));
...
static void select_toggle (PlaylistData * data, gboolean relative, gint position)
{
    ...
    mlp_playlist_entry_set_selected (active_playlist, position,
     ! mlp_playlist_entry_get_selected (active_playlist, position));
...
static void select_move (PlaylistData * data, gboolean relative, gint position)
{
    ...
    data->focused += mlp_playlist_shift (active_playlist, data->focused,
     position - data->focused);
    ...
    switch (event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK))
    {
      case 0:
        switch (event->keyval)
        {
          ...
          case GDK_Return:
            select_single (data, TRUE, 0);
            mlp_playlist_set_playing (active_playlist);
            mlp_playlist_set_position (active_playlist, data->focused);
            mlp_drct_play ();
            break;
          case GDK_Escape:
            select_single (data, FALSE, mlp_playlist_get_position
             (active_playlist));
            break;
          case GDK_Delete:
...
static gboolean playlist_button_press (GtkWidget * list, GdkEventButton * event)
{
    ...
    switch (event->type)
    {
      case GDK_BUTTON_PRESS:
        switch (event->button)
        {
          case 1:
            ...
            {
              case 0:
                if (mlp_playlist_entry_get_selected (active_playlist, position))
      ...
      case GDK_2BUTTON_PRESS:
        ...
        mlp_playlist_set_playing (active_playlist);
        mlp_playlist_set_position (active_playlist, position);
        mlp_drct_play ();
        break;
      default:
        return TRUE;
...
static gboolean playlist_motion (GtkWidget * list, GdkEventMotion * event)
{
    ...
    else
    {
        if (position == -1 || position == active_length)
            cancel_all (list, data);
        else if (mlp_get_bool (NULL, "show_filepopup_for_tuple") && data->popup_pos != position)
        {
            cancel_all (list, data);
...
static gboolean popup_show (GtkWidget * list)
{
    ...
    mlpgui_infopopup_show (active_playlist, data->popup_pos);
...
static void popup_trigger (GtkWidget * list, PlaylistData * data, gint pos)
{
    ...
    data->popup_pos = pos;
    data->popup_source = g_timeout_add (mlp_get_int (NULL, "filepopup_delay") *

...
static void saveconfig(void)
{
    ...
    if (pwd != NULL && pwd[0] != '\0') {
        ...
        mlp_set_string ("audioscrobbler", "password",
         hexify ((gchar *) md5pword, sizeof md5pword));
    }
    if (uid != NULL && uid[0] != '\0') {
        mlp_set_string ("audioscrobbler", "username", uid);
    } else {
        mlp_set_string ("audioscrobbler", "username", "");
        mlp_set_string ("audioscrobbler", "password", "");
    }

    if (url != NULL && url[0] != '\0')
        mlp_set_string ("audioscrobbler", "sc_url", url);
    else
        mlp_set_string ("audioscrobbler", "sc_url", LASTFM_HS_URL);
}

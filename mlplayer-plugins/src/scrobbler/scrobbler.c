    ...
    *(sc_srv_res + sc_srv_res_size) = 0;
    ...
    if (!strncmp(sc_srv_res, "UPDATE ", 7)) {
        ...
        *(sc_challenge_hash - 1) = 0;
        ...
        sc_throw_error(fmt_vastr("Please update MLPlayer.\n"
            "Update available at: http://mlplayer-media-player.org"));
...
static void read_cache(void)
{
    ...
    gchar * path = g_strconcat (mlp_get_path (AUD_PATH_USER_DIR), "/scrobblerqueue.txt", NULL);

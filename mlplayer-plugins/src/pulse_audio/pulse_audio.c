...
static int pulse_open(gint fmt, int rate, int nch) {
    ...
    if (!(context = pa_context_new(pa_threaded_mainloop_get_api(mainloop), "MLPlayer"))) {
    ...
    if (!(stream = pa_stream_new(context, "MLPlayer", &ss, NULL))) {
    ...
    int mlp_buffer = mlp_get_int(NULL, "output_buffer_size");
    size_t buffer_size = pa_usec_to_bytes(mlp_buffer, &ss) * 1000;
...
static void pulse_about(void) {
    ...
    mlpgui_simple_message(& dialog, GTK_MESSAGE_INFO,
            _("About MLPlayer PulseAudio Output Plugin"),
            "MLPlayer PulseAudio Output Plugin\n\n"

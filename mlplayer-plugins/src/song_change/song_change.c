...
static void do_command (char * cmd)
{
    ...
    int pos = mlp_playlist_get_position (playlist);
    ...
    if (cmd && strlen(cmd) > 0)
    {
        ...
        char * ctitle = mlp_playlist_entry_get_title (playlist, pos, FALSE);
        if (ctitle)
        {
        ...
        char * filename = mlp_playlist_entry_get_filename (playlist, pos);
        if (filename)
        {
        ...
        int length = mlp_playlist_entry_get_length (playlist, pos, FALSE);
        if (length > 0)
        {
        ...
        if (playing)
        {
            ...
            mlp_drct_get_info (& brate, & srate, & chans);
        ...
        Tuple * tuple = mlp_playlist_entry_get_tuple
            (mlp_playlist_get_active (), pos, 0);
...
{
    cmd_line = mlp_get_string("song_change", "cmd_line");
    cmd_line_after = mlp_get_string("song_change", "cmd_line_after");
    cmd_line_end = mlp_get_string("song_change", "cmd_line_end");
    cmd_line_ttc = mlp_get_string("song_change", "cmd_line_ttc");
}
...
{
    mlp_set_string("song_change", "cmd_line", cmd);
    mlp_set_string("song_change", "cmd_line_after", cmd_after);
    mlp_set_string("song_change", "cmd_line_end", cmd_end);
    mlp_set_string("song_change", "cmd_line_ttc", cmd_ttc);
...
{
    if ( ( mlp_ip_state->playing ) && ( strcmp(cmd_line_ttc,"") ) )
    {
        ...
        if ( ( prevs->title != NULL ) && ( prevs->filename != NULL ) )
        {
            if ( ( pl_entry->filename != NULL ) && ( !strcmp(pl_entry->filename,prevs->filename) ) )
            {
                ...
                {
                    int pos = mlp_drct_pl_get_pos();
                    char *current_file = mlp_drct_pl_get_file(pos);
...
static PreferencesWidget elements[] = {
    {WIDGET_LABEL, N_("Command to run when MLPlayer starts a new song."),
    ...
    {WIDGET_LABEL, N_("Command to run when MLPlayer reaches the end of the "

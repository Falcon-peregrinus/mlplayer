...
{
    mlp_config_set_defaults("null", null_defaults);
    return TRUE;
}
...
static void null_about(void)
{
    ...
    mlpgui_simple_message (& about, GTK_MESSAGE_INFO, _("About Null Output"),
     about_text);
...
{
    mlp_set_bool("null", "real_time", gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(data)));
...
static void null_configure(void)
{
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(rt_btn), mlp_get_bool("null", "real_time"));
...
static int null_open(gint fmt, int rate, int nch)
{
    ...
    if (mlp_get_bool("null", "real_time"))
        timer = g_timer_new();
    return 1;
}

...
static void oss_about (void)
{
    ...
    mlpgui_simple_message (& about_dialog, GTK_MESSAGE_INFO,
     _("About OSS Driver"),
     "MLPlayer OSS Driver\n\n "
     "This program is free software; you can redistribute it and/or modify\n"
     "it under the terms of the GNU General Public License as published by\n"
...
{
    mlp_config_set_defaults ("OSS", oss_defaults);

    memset(&oss_cfg, 0, sizeof(OSSConfig));

    oss_cfg.audio_device = mlp_get_int ("OSS", "audio_device");
    oss_cfg.mixer_device = mlp_get_int ("OSS", "mixer_device");
    oss_cfg.prebuffer = mlp_get_int ("OSS", "prebuffer");
    oss_cfg.use_master = mlp_get_bool ("OSS", "use_master");
    oss_cfg.use_alt_audio_device = mlp_get_bool ("OSS", "use_alt_audio_device");
    oss_cfg.alt_audio_device = mlp_get_string ("OSS", "alt_audio_device");
    oss_cfg.use_alt_mixer_device = mlp_get_bool ("OSS", "use_alt_mixer_device");
    oss_cfg.alt_mixer_device = mlp_get_string ("OSS", "alt_mixer_device");

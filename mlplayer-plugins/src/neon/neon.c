/*
 *  A neon HTTP input plugin for MLPlayer
...
{
    gchar * value = mlp_get_string (NULL, "proxy_user");
    g_strlcpy (username, value, NE_ABUFSIZ);
    g_free (value);

    value = mlp_get_string (NULL, "proxy_pass");
...
static gint open_handle(struct neon_handle* handle, gulong startbyte) {
    ...
    gboolean use_proxy = mlp_get_bool (NULL, "use_proxy");
    gboolean proxy_use_auth = mlp_get_bool (NULL, "proxy_use_auth");

    if (use_proxy)
    {
        proxy_host = mlp_get_string (NULL, "proxy_host");
        proxy_port = mlp_get_int (NULL, "proxy_port");
    }
    ...
    while (handle->redircount < 10) {
        ...
        ne_set_useragent(handle->session, "MLPlayer/" PACKAGE_VERSION );

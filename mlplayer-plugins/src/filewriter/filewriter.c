...
{
    mlp_config_set_defaults ("filewriter", filewriter_defaults);

    fileext = mlp_get_int ("filewriter", "fileext");
    filenamefromtags = mlp_get_bool ("filewriter", "filenamefromtags");
    file_path = mlp_get_string ("filewriter", "file_path");
    prependnumber = mlp_get_bool ("filewriter", "prependnumber");
    save_original = mlp_get_bool ("filewriter", "save_original");
    use_suffix = mlp_get_bool ("filewriter", "use_suffix");
...
void file_about (void)
{
    ...
    mlpgui_simple_message (& dialog, GTK_MESSAGE_INFO,
...
static gint file_open(gint fmt, gint rate, gint nch)
{
    ...
    playlist = mlp_playlist_get_playing ();
    if (playlist < 0)
        return 0;

    pos = mlp_playlist_get_position(playlist);
    tuple = mlp_playlist_entry_get_tuple (playlist, pos, FALSE);
    if (tuple == NULL)
        return 0;

    if (filenamefromtags)
    {
        gchar * utf8 = mlp_playlist_entry_get_title (playlist, pos, FALSE);
    ...
    {
        temp = mlp_playlist_entry_get_filename (playlist, pos);
...
static void configure_ok_cb(gpointer data)
{
    ...
    mlp_set_int ("filewriter", "fileext", fileext);
    mlp_set_bool ("filewriter", "filenamefromtags", filenamefromtags);
    mlp_set_string ("filewriter", "file_path", file_path);
    mlp_set_bool ("filewriter", "prependnumber", prependnumber);
    mlp_set_bool ("filewriter", "save_original", save_original);
    mlp_set_bool ("filewriter", "use_suffix", use_suffix);

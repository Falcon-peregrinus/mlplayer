...
static GtkStatusIcon *si_create(void)
{
    ...
    if (gtk_icon_theme_has_icon(theme, "mlplayer-panel"))
        icon = gtk_status_icon_new_from_icon_name("mlplayer-panel");
    else if (gtk_icon_theme_has_icon(theme, "mlplayer"))
        icon = gtk_status_icon_new_from_icon_name("mlplayer");
    else
    {
        gchar * path = g_strdup_printf ("%s/images/mlplayer_player.xpm",
         mlp_get_path (AUD_PATH_DATA_DIR));
    ...
    switch (event->button)
    {
      case 1:
      {
          if (event->state & GDK_SHIFT_MASK)
              ...
              mlp_interface_show (! (mlp_interface_is_shown () &&
               mlp_interface_is_focused ()));
          break;
      }
      ...
      case GDK_SCROLL_UP:
      {
          switch (si_cfg.scroll_action)
          {
            ...
            case SI_CFG_SCROLL_ACTION_SKIP:
                si_playback_skip (mlp_get_bool ("statusicon", "reverse_scroll") ? 1 : -1);
                break;
          }
          break;
      }
...
static GtkWidget *si_smallmenu_create(void)
{
    ...
    if (si_cfg.rclick_menu == SI_CFG_RCLICK_MENU_SMALL2)
    {
        ...
        g_signal_connect_swapped(si_smenu_quit_item, "activate", G_CALLBACK(mlp_drct_quit), NULL);
...
static void si_enable(gboolean enable)
{
    ...
    if (enable && ! si_applet)
    {
        ...
        /* small menu that can be used in place of the mlplayer standard one */
    ...
    if (! enable && si_applet)
    {
         ...
         * by disabling the plugin while MLPlayer is closed to the tray. */
        extern GeneralPlugin _mlp_plugin_self;
        PluginHandle *si = mlp_plugin_by_header(&_mlp_plugin_self);
        if (! mlp_plugin_get_enabled(si) && ! mlp_interface_is_shown())
            mlp_interface_show(TRUE);
...
void si_about(void)
{
    ...
    mlpgui_simple_message(&about_dlg, GTK_MESSAGE_INFO, _("About Status Icon Plugin"),
...
void si_prefs_cb_commit(gpointer prefs_win)
{
    ...
    mlp_set_bool ("statusicon", "reverse_scroll", gtk_toggle_button_get_active
...
void si_config(void)
{
    ...
    gtk_toggle_button_set_active ((GtkToggleButton *) reverse_scroll_toggle,
     mlp_get_bool ("statusicon", "reverse_scroll"));

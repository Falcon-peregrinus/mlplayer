...
{
  mlp_config_set_defaults (CFG_VERSION, adplug_defaults);

  conf.bit16 = mlp_get_bool (CFG_VERSION, "16bit");
  conf.stereo = mlp_get_bool (CFG_VERSION, "Stereo");
  conf.freq = mlp_get_int (CFG_VERSION, "Frequency");
  conf.endless = mlp_get_bool (CFG_VERSION, "Endless");

  // Read file type exclusion list
  dbg_printf ("exclusion, ");
  {
    gchar * cfgstr = mlp_get_string (CFG_VERSION, "Exclude");
...
adplug_quit (void)
{
  ...
  mlp_set_bool (CFG_VERSION, "16bit", conf.bit16);
  mlp_set_bool (CFG_VERSION, "Stereo", conf.stereo);
  mlp_set_int (CFG_VERSION, "Frequency", conf.freq);
  mlp_set_bool (CFG_VERSION, "Endless", conf.endless);
  ...
  mlp_set_string (CFG_VERSION, "Exclude", exclude.c_str ());

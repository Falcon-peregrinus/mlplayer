...
static void update (void * unused, void * explicit)
{
    if (! mlp_drct_get_playing () || ! mlp_drct_get_ready ())
    {
        if (GPOINTER_TO_INT (explicit))
            osd_show (_("Stopped"), _("MLPlayer is not playing."), NULL, NULL);

        return;
    }

    gint list = mlp_playlist_get_playing ();
    gint entry = mlp_playlist_get_position (list);

    gchar * title, * artist, * album;
    mlp_playlist_entry_describe (list, entry, & title, & artist, & album, FALSE);
    ...
    GdkPixbuf * pb = mlpgui_pixbuf_for_current ();
    if (pb)
        mlpgui_pixbuf_scale_within (& pb, 96);

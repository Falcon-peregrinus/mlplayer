/*
 *  This file is part of mlplayer-hotkey plugin for mlplayer
 *
 ...
 *
 *  mlplayer-hotkey is free software; you can redistribute it and/or modify
 *  it under the terms of the GNU General Public License as published by
 *  the Free Software Foundation; either version 2 of the License, or
 *  (at your option) any later version.
 *
 *  mlplayer-hotkey is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU General Public License for more details.
 *
 *  You should have received a copy of the GNU General Public License
 *  along with mlplayer-hotkey; if not, write to the Free Software
...
gboolean handle_keyevent (EVENT event)
{
	...
	/* playing or not */
	play = mlp_drct_get_playing ();

	/* get current volume */
	mlp_drct_get_volume_main (&current_volume);
	...
	if (event == EVENT_MUTE)
	{
		if (!mute)
		{
			...
			mlp_drct_set_volume_main (0);
			mute = TRUE;
		} else {
			mlp_drct_set_volume_main (volume_static);
			mute = FALSE;
		}
		return TRUE;
	}
	...
	if (event == EVENT_VOL_DOWN)
	{
		...
		{
			mlp_drct_set_volume_main (current_volume);
		}
	...
	{
		mlp_interface_show_jump_to_track ();
		return TRUE;
	}
	...
	{
		mlp_interface_show (! (mlp_interface_is_shown () && mlp_interface_is_focused ()));
		return TRUE;
	}
	...
	if (event == EVENT_SHOW_AOSD)
	{
		...
		mlp_set_bool (NULL, "repeat", ! mlp_get_bool (NULL, "repeat"));
	else if (event == EVENT_TOGGLE_SHUFFLE)
		mlp_set_bool (NULL, "shuffle", ! mlp_get_bool (NULL, "shuffle"));
...
void load_config (void)
{
	...
	max = mlp_get_int ("globalHotkey", "NumHotkeys");
	if (max == 0)
		load_defaults();
	else for (i=0; i<max; i++)
	{
		...
		hotkey->key = mlp_get_int ("globalHotkey", text);
		g_free(text);

		text = g_strdup_printf("Hotkey_%d_mask", i);
		hotkey->mask = mlp_get_int ("globalHotkey", text);
		g_free(text);

		text = g_strdup_printf("Hotkey_%d_type", i);
		hotkey->type = mlp_get_int ("globalHotkey", text);
		g_free(text);

		text = g_strdup_printf("Hotkey_%d_event", i);
		hotkey->event = mlp_get_int ("globalHotkey", text);
		g_free(text);
	}
}
...
void save_config (void)
{
	...
	while (hotkey) {
		...
		if (hotkey->key) {
			...
			mlp_set_int ("globalHotkey", text, hotkey->key);
			g_free(text);

			text = g_strdup_printf("Hotkey_%d_mask", max);
			mlp_set_int ("globalHotkey", text, hotkey->mask);
			g_free(text);

			text = g_strdup_printf("Hotkey_%d_type", max);
			mlp_set_int ("globalHotkey", text, hotkey->type);
			g_free(text);

			text = g_strdup_printf("Hotkey_%d_event", max);
			mlp_set_int ("globalHotkey", text, hotkey->event);
			g_free(text);
			max++;
		}
	...
	mlp_set_int ("globalHotkey", "NumHotkeys", max);
}

...
static gboolean ui_infoarea_do_fade (void)
{
    ...
    if (mlp_drct_get_playing () && area->alpha < 1)
    {
        area->alpha += 0.1;
        ret = TRUE;
    }
...
static void set_album_art (void)
{
    ...
    area->pb = mlpgui_pixbuf_for_current ();
    if (area->pb)
        mlpgui_pixbuf_scale_within (& area->pb, ICON_SIZE);
}
...
static void destroy_cb (GtkWidget * widget)
{
    ...
    mlp_vis_func_remove ((VisFunc) vis_render_cb);
...
GtkWidget * ui_infoarea_new (void)
{
    ...
    mlp_vis_func_add (AUD_VIS_TYPE_FREQ, (VisFunc) vis_render_cb);

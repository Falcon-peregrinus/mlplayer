...
{
    PluginHandle * plugin = mlp_plugin_by_widget (widget);
    g_return_if_fail (plugin);
    mlp_plugin_enable (plugin, FALSE);
}
...
void layout_save (void)
{
    ...
    for (GList * node = items; node; node = node->next)
    {
        ...
        mlp_set_string ("gtkui-layout", key, item->name);

        snprintf (key, sizeof key, "item%d_pos", i);
        snprintf (value, sizeof value, "%d,%d,%d,%d,%d", item->dock, item->x,
         item->y, item->w, item->h);
        mlp_set_string ("gtkui-layout", key, value);

        i ++;
    }

    mlp_set_int ("gtkui-layout", "item_count", i);
}
...
void layout_load (void)
{
    ...
    gint count = mlp_get_int ("gtkui-layout", "item_count");
    ...
    for (gint i = 0; i < count; i ++)
    {
        ...
        snprintf (key, sizeof key, "item%d_name", i);
        gchar * name = mlp_get_string ("gtkui-layout", key);
        Item * item = item_new (name);
        g_free (name);

        snprintf (key, sizeof key, "item%d_pos", i);
        gchar * pos = mlp_get_string ("gtkui-layout", key);

...
void pw_col_init (void)
{
    ...
    gchar * columns = mlp_get_string ("gtkui", "playlist_columns");
...
static void shift_rows (void * user, gint row, gint before)
{
    ...
    mlpgui_list_update_rows (list, begin, end - begin);
    mlpgui_list_update_selection (list, begin, end - begin);
}
...
static void transfer (Index * source)
{
    ...
    for (gint row = 0; row < source_rows; )
    {
        ...
        mlpgui_list_delete_rows (source_list, row, 1);
        source_rows --;
        index_append (dest, c);
        mlpgui_list_insert_rows (dest_list, dest_rows, 1);
        dest_rows ++;
    }
}
...
void pw_col_choose (void)
{
    ...
    avail_list = mlpgui_list_new (& callbacks, avail, index_count (avail));
    gtk_tree_view_set_headers_visible ((GtkTreeView *) avail_list, FALSE);
    mlpgui_list_add_column (avail_list, NULL, 0, G_TYPE_STRING, -1);
    ...
    chosen_list = mlpgui_list_new (& callbacks, chosen, index_count (chosen));
    gtk_tree_view_set_headers_visible ((GtkTreeView *) chosen_list, FALSE);
    mlpgui_list_add_column (chosen_list, NULL, 0, G_TYPE_STRING, -1);
...
void pw_col_save (void)
{
    ...
    mlp_set_string ("gtkui", "playlist_columns", s->str);

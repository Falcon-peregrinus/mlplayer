 ...
 *  Copyright (C) 2005-2012  MLPlayer development team
 *
...
{
    mlpgui_confirm_playlist_delete (mlp_playlist_by_unique_id (GPOINTER_TO_INT (id)));
}
...
static GtkWidget * make_close_button (gint list)
{
    ...
    g_signal_connect (button, "clicked", (GCallback) close_button_cb,
     GINT_TO_POINTER (mlp_playlist_get_unique_id (list)));
...
static void tab_title_save(GtkEntry *entry, gpointer ebox)
{
    ...
    mlp_playlist_set_title(mlp_playlist_get_active(), gtk_entry_get_text(entry));
...
static gboolean tab_button_press_cb(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    if (event->type == GDK_BUTTON_PRESS && event->button == 2)
    {
        ...
        mlpgui_confirm_playlist_delete(gtk_notebook_page_num(UI_PLAYLIST_NOTEBOOK, page));
    }
 ...
 page_num, void * unused)
{
    ...
    if (treeview != NULL)
    {
        mlp_playlist_set_active (page_num);
...
static void tab_reordered(GtkNotebook *notebook, GtkWidget *child, guint page_num, gpointer user_data)
{
    ...
    mlp_playlist_reorder (ui_playlist_widget_get_playlist (widget), page_num, 1);
}
...
{
    gchar * title = mlp_playlist_get_title (list);

    if (list == mlp_playlist_get_playing ())
    {
...
void ui_playlist_notebook_edit_tab_title(GtkWidget *ebox)
{
    ...
    {
        GtkWidget *page = gtk_notebook_get_nth_page(UI_PLAYLIST_NOTEBOOK, mlp_playlist_get_active());
    ...
    gchar * title = mlp_playlist_get_title (mlp_playlist_get_active ());
...
void ui_playlist_notebook_create_tab(gint playlist)
{
    ...
    gint position = mlp_playlist_get_position (playlist);
    ...
    g_object_set_data ((GObject *) treeview, "playlist-id",
     GINT_TO_POINTER (mlp_playlist_get_unique_id (playlist)));

    if (position >= 0)
    {
        mlp_playlist_select_all (playlist, FALSE);
        mlp_playlist_entry_set_selected (playlist, position, TRUE);
        mlpgui_list_set_highlight (treeview, position);
        mlpgui_list_set_focus (treeview, position);
    }
...
void ui_playlist_notebook_populate(void)
{
    ...
    gtk_notebook_set_current_page (UI_PLAYLIST_NOTEBOOK, mlp_playlist_get_active ());
    highlighted = mlp_playlist_get_unique_id (mlp_playlist_get_playing ());
...
static void do_follow (void)
{
    ...
    {
        gint list = mlp_playlist_by_unique_id (GPOINTER_TO_INT (g_queue_pop_head
         (& follow_queue)));
        ...
        if (row == CURRENT_POS)
        {
            row = mlp_playlist_get_position (list);
            mlpgui_list_set_highlight (widget, row);

            if (! mlp_get_bool ("gtkui", "autoscroll"))
                continue;
        }

        mlpgui_list_set_focus (widget, row);
    }
}

static void add_remove_pages (void)
{
    gint lists = mlp_playlist_count ();
    ...
    for (gint i = 0; i < pages; )
    {
        ...
        if (mlp_playlist_by_unique_id (tree_id) < 0)
        {
        ...
        gint list_id = mlp_playlist_get_unique_id (i);
...
void ui_playlist_notebook_update (void * data, void * user)
{
    ...
    for (gint list = 0; list < lists; list ++)
    {
        ...
        gint at, count;
        gint level = mlp_playlist_updated_range (list, & at, & count);

        if (level)
            ui_playlist_widget_update (playlist_get_treeview (list), level, at, count);
    }

    gtk_notebook_set_current_page ((GtkNotebook *) notebook, mlp_playlist_get_active ());
...
void playlist_set_focus (gint list, gint row)
{
    g_queue_push_tail (& follow_queue, GINT_TO_POINTER
     (mlp_playlist_get_unique_id (list)));
...
void ui_playlist_notebook_position (void * data, void * user)
{
    ...
    if (mlp_get_bool ("gtkui", "autoscroll"))
    {
        ...
        if (mlp_playlist_get_position (list) >= 0)
            mlp_playlist_entry_set_selected (list, mlp_playlist_get_position (list), TRUE);
    }
...
{
    gint new = mlp_playlist_get_unique_id (mlp_playlist_get_playing ());
...
{
    gboolean old = mlp_get_bool ("gtkui", "playlist_headers");
    if (old == show)
        return;

    mlp_set_bool ("gtkui", "playlist_headers", show);

...
static void save_window_size (void)
{
    ...
    mlp_set_int ("gtkui", "player_x", x);
    mlp_set_int ("gtkui", "player_y", y);
    mlp_set_int ("gtkui", "player_width", w);
    mlp_set_int ("gtkui", "player_height", h);
}
...
{
    mlpgui_run_filebrowser(TRUE);
}

static void button_add_pressed()
{
    mlpgui_run_filebrowser(FALSE);
}
...
static gboolean title_change_cb (void)
{
    ...
    if (mlp_drct_get_playing () && mlp_get_bool ("gtkui", "show_song_titles"))
    {
        if (mlp_drct_get_ready ())
        {
            gchar * title = mlp_drct_get_title ();
            gchar * title_s = g_strdup_printf (_("%s - MLPlayer"), title);
        ...
        gtk_window_set_title ((GtkWindow *) window, _("MLPlayer"));
...
static void ui_show (gboolean show)
{
    mlp_set_bool ("gtkui", "player_visible", show);

    if (show)
    {
        if (mlp_get_bool ("gtkui", "save_window_position") && ! gtk_widget_get_visible (window))
        {
            gint x = mlp_get_int ("gtkui", "player_x");
            gint y = mlp_get_int ("gtkui", "player_y");
    ...
    {
        if (mlp_get_bool ("gtkui", "save_window_position"))
        {
            gint x, y;
            gtk_window_get_position ((GtkWindow *) window, & x, & y);
            mlp_set_int ("gtkui", "player_x", x);
...
static gboolean ui_is_shown (void)
{
    return mlp_get_bool ("gtkui", "player_visible");
}
...
static void set_time_label (gint time, gint len)
{
    ...
    if (time < 3600)
        snprintf (s + strlen (s), sizeof s - strlen (s), mlp_get_bool (NULL,
...
static gboolean time_counter_cb (void)
{
    ...
    gint time = mlp_drct_get_time ();
    gint length = mlp_drct_get_length ();
...
{
    set_slider (time);
    set_time_label (time, mlp_drct_get_length ());
    mlp_drct_seek (time);
...
static gboolean ui_slider_change_value_cb(GtkRange * range, GtkScrollType scroll)
{
    ...
    set_time_label (value, mlp_drct_get_length ());
...
{
    mlp_drct_set_volume((gint) volume, (gint) volume);
...
static gboolean ui_volume_slider_update(gpointer data)
{
    ...
    mlp_drct_get_volume_main(&volume);
...
{
    title_change_cb ();
    set_slider_length (mlp_drct_get_length ());
    time_counter_cb ();
...
{
    gtk_widget_grab_focus (playlist_get_treeview (mlp_playlist_get_active ()));
}
    ...
    switch (event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK))
    {
      case 0:;
        ...
        case GDK_Left:
            if (mlp_drct_get_playing ())
                do_seek (mlp_drct_get_time () - 5000);
            return TRUE;
        case GDK_Right:
            if (mlp_drct_get_playing ())
                do_seek (mlp_drct_get_time () + 5000);
      ...
      case (GDK_CONTROL_MASK | GDK_SHIFT_MASK):
        switch (event->keyval)
        {
          case GDK_ISO_Left_Tab:
          case GDK_Tab:
            mlp_playlist_set_active (mlp_playlist_get_active () ?
             mlp_playlist_get_active () - 1 : mlp_playlist_count () - 1);
            break;
          default:
            return FALSE;
        }
...
static gboolean playlist_keypress_cb (GtkWidget * widget, GdkEventKey * event, void * unused)
{
    ...
    case 0:
        ...
        {
        case GDK_Escape:
            ui_playlist_notebook_position (GINT_TO_POINTER (mlp_playlist_get_active ()), NULL);
            return TRUE;
        case GDK_Delete:
    ...
    case GDK_CONTROL_MASK:
        ...
        case 'v':
            ...
            return TRUE;
        case 'a':
            mlp_playlist_select_all (mlp_playlist_get_active (), TRUE);
            return TRUE;
        }
...
{
    gtk_toggle_button_set_active ((GtkToggleButton *) button_repeat, mlp_get_bool (NULL, "repeat"));
    gtk_toggle_button_set_active ((GtkToggleButton *) button_shuffle, mlp_get_bool (NULL, "shuffle"));
}

static void toggle_repeat (GtkToggleButton * button, void * unused)
{
    mlp_set_bool (NULL, "repeat", gtk_toggle_button_get_active (button));
}

static void toggle_shuffle (GtkToggleButton * button, void * unused)
{
    mlp_set_bool (NULL, "shuffle", gtk_toggle_button_get_active (button));
}

static void toggle_search_tool (GtkToggleButton * button, void * unused)
{
    mlp_plugin_enable (search_tool, gtk_toggle_button_get_active (button));
}
...
static gboolean init (void)
{
    search_tool = mlp_plugin_lookup_basename ("search-tool");
    ...
    mlp_config_set_defaults ("gtkui", gtkui_defaults);
    ...
    gint x = mlp_get_int ("gtkui", "player_x");
    gint y = mlp_get_int ("gtkui", "player_y");
    gint w = mlp_get_int ("gtkui", "player_width");
    gint h = mlp_get_int ("gtkui", "player_height");

    window = gtk_window_new (GTK_WINDOW_TOPLEVEL);
    gtk_window_set_default_size ((GtkWindow *) window, w, h);
    gtk_window_set_keep_above ((GtkWindow *) window, mlp_get_bool ("gtkui", "always_on_top"));
#if GTK_CHECK_VERSION (3, 0, 0)
    gtk_window_set_has_resize_grip ((GtkWindow *) window, FALSE);
#endif

    if (mlp_get_bool ("gtkui", "save_window_position") && (x != -1 || y != -1))
        gtk_window_move ((GtkWindow *) window, x, y);
    ...
    show_menu (mlp_get_bool ("gtkui", "menu_visible"));
    ...
    if (search_tool)
    {
        ...
        gtk_toggle_button_set_active ((GtkToggleButton *) search_button,
         mlp_plugin_get_enabled (search_tool));
        mlp_plugin_add_watch (search_tool, search_tool_toggled, NULL);
    }
    ...
    button_play = toolbar_button_add (buttonbox, mlp_drct_play, GTK_STOCK_MEDIA_PLAY);
    button_pause = toolbar_button_add (buttonbox, mlp_drct_pause, GTK_STOCK_MEDIA_PAUSE);
    button_stop = toolbar_button_add (buttonbox, mlp_drct_stop, GTK_STOCK_MEDIA_STOP);
    toolbar_button_add (buttonbox, mlp_drct_pl_prev, GTK_STOCK_MEDIA_PREVIOUS);
    toolbar_button_add (buttonbox, mlp_drct_pl_next, GTK_STOCK_MEDIA_NEXT);
    ...
    if (mlp_get_bool ("gtkui", "infoarea_visible"))
    {
        infoarea = ui_infoarea_new ();
        gtk_box_pack_end (GTK_BOX(vbox), infoarea, FALSE, FALSE, 0);
    }

    if (mlp_get_bool ("gtkui", "statusbar_visible"))
    {
    ...
    if (mlp_get_bool ("gtkui", "player_visible"))
        ui_show (TRUE);
...
static void cleanup (void)
{
    ...
    if (search_tool)
        mlp_plugin_remove_watch (search_tool, search_tool_toggled, NULL);
...
{
    mlp_set_bool ("gtkui", "menu_visible", show);
...
{
    mlp_set_bool ("gtkui", "infoarea_visible", show);
...
{
    mlp_set_bool ("gtkui", "statusbar_visible", show);

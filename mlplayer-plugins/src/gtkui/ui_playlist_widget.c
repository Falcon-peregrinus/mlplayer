...
{
    int q = mlp_playlist_queue_find_entry (list, row);
    if (q < 0)
        g_value_set_string (value, "");
    else
...
{
    gint len = mlp_playlist_entry_get_length (list, row, TRUE);
    if (len)
    {
        len /= 1000;
        ...
        gchar s[16];
        if (len < 3600)
            snprintf (s, sizeof s, mlp_get_bool (NULL, "leading_zero") ?
...
static void get_value (void * user, gint row, gint column, GValue * value)
{
    ...
    g_return_if_fail (row >= 0 && row < mlp_playlist_entry_count (data->list));
    ...
    case PW_COL_PATH:
        ...
        break;
    case PW_COL_CUSTOM:;
        gchar * custom = mlp_playlist_entry_get_title (data->list, row, TRUE);
...
{
    return mlp_playlist_entry_get_selected (((PlaylistWidgetData *) user)->list,
     row);
}

static void set_selected (void * user, gint row, gboolean selected)
{
    mlp_playlist_entry_set_selected (((PlaylistWidgetData *) user)->list, row,
     selected);
}

static void select_all (void * user, gboolean selected)
{
    mlp_playlist_select_all (((PlaylistWidgetData *) user)->list, selected);
}

static void activate_row (void * user, gint row)
{
    gint list = ((PlaylistWidgetData *) user)->list;
    mlp_playlist_set_playing (list);
    mlp_playlist_set_position (list, row);
    mlp_drct_play ();
}
...
static void shift_rows (void * user, gint row, gint before)
{
    ...
    mlp_playlist_shift (list, row, before - row);
}

static void get_data (void * user, void * * data, gint * length)
{
    gchar * text = mlpgui_urilist_create_from_selected
...
static void receive_data (void * user, gint row, const void * data, gint length)
{
    ...
    text[length] = 0;
    mlpgui_urilist_insert (((PlaylistWidgetData *) user)->list, row, text);
    g_free (text);
}
 ...
 GtkTreeIter * iter, void * user)
{
    ...
    mlp_playlist_entry_describe (((PlaylistWidgetData *) user)->list, row,
     & s[0], & s[1], & s[2], FALSE);
...
GtkWidget * ui_playlist_widget_new (gint playlist)
{
    ...
    GtkWidget * list = mlpgui_list_new (& callbacks, data,
     mlp_playlist_entry_count (playlist));

    if (mlp_get_bool ("gtkui", "custom_playlist_colors"))
    {
        GdkColor c;
        gchar * tmp;

        tmp = mlp_get_string ("gtkui", "playlist_bg");
        gdk_color_parse (tmp, & c);
        gtk_widget_modify_base(list, GTK_STATE_NORMAL, &c);

        tmp = mlp_get_string ("gtkui", "playlist_fg");
        gdk_color_parse (tmp, & c);
        gtk_widget_modify_text(list, GTK_STATE_NORMAL, &c);
    }

    gchar * font = mlp_get_string ("gtkui", "playlist_font");
    if (font[0])
    {
    ...
    gtk_tree_view_set_headers_visible ((GtkTreeView *) list,
     mlp_get_bool ("gtkui", "playlist_headers"));
    ...
    {
        gint n = pw_cols[i];
        mlpgui_list_add_column (list, pw_col_label[n] ? _(pw_col_names[n]) :
...
{
    PlaylistWidgetData * data = mlpgui_list_get_user (widget);
...
static void update_queue (GtkWidget * widget, PlaylistWidgetData * data)
{
    for (GList * node = data->queue; node; node = node->next)
        mlpgui_list_update_rows (widget, GPOINTER_TO_INT (node->data), 1);

    g_list_free (data->queue);
    data->queue = NULL;

    for (gint i = mlp_playlist_queue_count (data->list); i --; )
        data->queue = g_list_prepend (data->queue, GINT_TO_POINTER
         (mlp_playlist_queue_get_entry (data->list, i)));
 ...
 gint count)
{
    ...
    {
        gint diff = mlp_playlist_entry_count (data->list) -
         mlpgui_list_row_count (widget);

        if (diff > 0)
            mlpgui_list_insert_rows (widget, at, diff);
        else if (diff < 0)
            mlpgui_list_delete_rows (widget, at, -diff);

        mlpgui_list_set_highlight (widget, mlp_playlist_get_position (data->list));
    }

    if (type >= PLAYLIST_UPDATE_METADATA)
        mlpgui_list_update_rows (widget, at, count);

    mlpgui_list_update_selection (widget, at, count);

 ...
 *  Copyright (C) 2010-2011 MLPlayer development team
 *
...
ui_statusbar_update_playlist_length(gpointer unused, GtkWidget *label)
{
    ...
    total = mlp_playlist_get_total_length (playlist) / 1000;
    selection = mlp_playlist_get_selected_length (playlist) / 1000;
...
ui_statusbar_info_change(gpointer unused, GtkWidget *label)
{
    ...
    Tuple * tuple = mlp_playlist_entry_get_tuple (playlist,
     mlp_playlist_get_position (playlist), FALSE);
...
ui_statusbar_new(void)
{
    ...
    if (mlp_drct_get_playing() && mlp_drct_get_ready())

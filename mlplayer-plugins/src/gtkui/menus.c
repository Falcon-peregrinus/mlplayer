...
static void open_files (void) {mlpgui_run_filebrowser (TRUE); }
static void open_url (void) {mlpgui_show_add_url_window (TRUE); }
static void add_files (void) {mlpgui_run_filebrowser (FALSE); }
static void add_url (void) {mlpgui_show_add_url_window (FALSE); }

static gboolean repeat_get (void) {return mlp_get_bool (NULL, "repeat"); }
static void repeat_set (gboolean on) {mlp_set_bool (NULL, "repeat", on); }
static gboolean shuffle_get (void) {return mlp_get_bool (NULL, "shuffle"); }
static void shuffle_set (gboolean on) {mlp_set_bool (NULL, "shuffle", on); }
static gboolean no_advance_get (void) {return mlp_get_bool (NULL, "no_playlist_advance"); }
static void no_advance_set (gboolean on) {mlp_set_bool (NULL, "no_playlist_advance", on); }
static gboolean stop_after_get (void) {return mlp_get_bool (NULL, "stop_after_current_song"); }
static void stop_after_set (gboolean on) {mlp_set_bool (NULL, "stop_after_current_song", on); }

static void pl_sort_track (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_TRACK); }
static void pl_sort_title (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_TITLE); }
static void pl_sort_artist (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_ARTIST); }
static void pl_sort_album (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_ALBUM); }
static void pl_sort_date (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_DATE); }
static void pl_sort_path (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_PATH); }
static void pl_sort_custom (void) {mlp_playlist_sort_by_scheme (mlp_playlist_get_active (), PLAYLIST_SORT_FORMATTED_TITLE); }
static void pl_reverse (void) {mlp_playlist_reverse (mlp_playlist_get_active ()); }
static void pl_random (void) {mlp_playlist_randomize (mlp_playlist_get_active ()); }

static void pl_new (void)
{
    mlp_playlist_insert (-1);
    mlp_playlist_set_active (mlp_playlist_count () - 1);
}

static void pl_refresh (void) {mlp_playlist_rescan (mlp_playlist_get_active ()); }
static void pl_remove_failed (void) {mlp_playlist_remove_failed (mlp_playlist_get_active ()); }
static void pl_close (void) {mlpgui_confirm_playlist_delete (mlp_playlist_get_active ()); }
static void pl_refresh_sel (void) {mlp_playlist_rescan_selected (mlp_playlist_get_active ()); }
static void pl_select_all (void) {mlp_playlist_select_all (mlp_playlist_get_active (), TRUE); }
static void pl_rename (void) {ui_playlist_notebook_edit_tab_title (NULL); }

static GtkWidget * get_services_main (void) {return mlp_get_plugin_menu (AUD_MENU_MAIN); }
static GtkWidget * get_services_pl (void) {return mlp_get_plugin_menu (AUD_MENU_PLAYLIST_RCLICK); }

static void volume_up (void)
{
    int vol = 0;
    mlp_drct_get_volume_main (& vol);
    mlp_drct_set_volume_main (vol + 5);
}
...
static gboolean menu_bar_get (void) {return mlp_get_bool ("gtkui", "menu_visible"); }
static gboolean info_bar_get (void) {return mlp_get_bool ("gtkui", "infoarea_visible"); }
static gboolean status_bar_get (void) {return mlp_get_bool ("gtkui", "statusbar_visible"); }
static gboolean column_headers_get (void) {return mlp_get_bool ("gtkui", "playlist_headers"); }
static gboolean autoscroll_get (void) {return mlp_get_bool ("gtkui", "autoscroll"); }
static void autoscroll_set (gboolean on) {mlp_set_bool ("gtkui", "autoscroll", on); }
...
static const struct MenuItem file_items[] = {
 ...
 {.sep = TRUE},
 {N_("A_bout ..."), GTK_STOCK_ABOUT, .func = mlpgui_show_about_window},
 {N_("_Preferences ..."), GTK_STOCK_PREFERENCES, .func = mlp_show_prefs_window},
 {N_("_Quit"), GTK_STOCK_QUIT, 'q', CTRL, .func = mlp_drct_quit}};

static const struct MenuItem playback_items[] = {
 {N_("_Play"), GTK_STOCK_MEDIA_PLAY, GDK_Return, CTRL, .func = mlp_drct_play},
 {N_("Paus_e"), GTK_STOCK_MEDIA_PAUSE, ',', CTRL, .func = mlp_drct_pause},
 {N_("_Stop"), GTK_STOCK_MEDIA_STOP, '.', CTRL, .func = mlp_drct_stop},
 {N_("Pre_vious"), GTK_STOCK_MEDIA_PREVIOUS, GDK_Up, ALT, .func = mlp_drct_pl_prev},
 {N_("_Next"), GTK_STOCK_MEDIA_NEXT, GDK_Down, ALT, .func = mlp_drct_pl_next},
 {.sep = TRUE},
 ...
 {.sep = TRUE},
 {N_("Song _Info ..."), GTK_STOCK_INFO, 'i', CTRL, .func = mlpgui_infowin_show_current},
 {N_("Jump to _Time ..."), GTK_STOCK_JUMP_TO, .func = mlpgui_jump_to_time},
 {N_("_Jump to Song ..."), GTK_STOCK_JUMP_TO, 'j', CTRL, .func = mlpgui_jump_to_track}};
...
static const struct MenuItem playlist_items[] = {
 ...
 {.sep = TRUE},
 {N_("_Import ..."), GTK_STOCK_OPEN, .func = mlpgui_import_playlist},
 {N_("_Export ..."), GTK_STOCK_SAVE, .func = mlpgui_export_playlist},
 {.sep = TRUE},
 {N_("_Playlist Manager ..."), AUD_STOCK_PLAYLIST, 'p', CTRL, .func = mlpgui_playlist_manager},
 {N_("_Queue Manager ..."), AUD_STOCK_QUEUETOGGLE, 'u', CTRL, .func = mlpgui_queue_manager_show}};
...
static const struct MenuItem output_items[] = {
 ...
 {.sep = TRUE},
 {N_("_Equalizer"), GTK_STOCK_PREFERENCES, 'e', CTRL, .func = mlpgui_show_equalizer_window},
 {.sep = TRUE},
 {N_("E_ffects"), .get_sub = mlpgui_create_effects_menu}};

static const struct MenuItem view_items[] = {
 {N_("_Interface"), .get_sub = mlpgui_create_iface_menu},
 {N_("_Visualizations"), .get_sub = mlpgui_create_vis_menu},
 {.sep = TRUE},

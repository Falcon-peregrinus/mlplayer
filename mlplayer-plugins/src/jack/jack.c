 ...
 *
 *      mlplayer port (2005-2006) by Giacomo Lozito from develia.org
 *
...
/* Giacomo's note: removed the destructor from the original xmms-jack, cause
   ...
   by adding a cleanup function callback for output plugins in MLPlayer, this
...
{
  mlp_config_set_defaults ("jack", jack_defaults);

  jack_cfg.isTraceEnabled = mlp_get_bool ("jack", "isTraceEnabled");
  jack_cfg.port_connection_mode = mlp_get_string ("jack", "port_connection_mode");
  jack_cfg.volume_left = mlp_get_int ("jack", "volume_left");
  jack_cfg.volume_right = mlp_get_int ("jack", "volume_right");
  ...
  /* jack client list */
  JACK_SetClientName("mlplayer-jack");
...
static gint mlplayer_jack_free(void)
{
...
{
  mlp_set_int ("jack", "volume_left", jack_cfg.volume_left);
  mlp_set_int ("jack", "volume_right", jack_cfg.volume_right);
...
(
    ...
    .flush = jack_flush,
    .pause = jack_pause,
    .buffer_free = mlplayer_jack_free,

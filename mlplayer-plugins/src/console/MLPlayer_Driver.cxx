...
static Tuple * get_track_ti(const gchar *path, const track_info_t *info, const gint track)
{
    ...
    if (ti != NULL)
    {
        ...
        if (length <= 0)
            length = mlpcfg.loop_length * 1000;
    ...
    VFSFile *file, gint start_time, gint stop_time, gboolean pause)
{
    ...
    if (fh.m_type == gme_spc_type)
        sample_rate = 32000;
    if (mlpcfg.resample)
        sample_rate = mlpcfg.resample_rate;
    if (sample_rate == 0)
        sample_rate = 44100;
    ...
    // stereo echo depth
    gme_set_stereo_depth(fh.m_emu, 1.0 / 100 * mlpcfg.echo);

    // set equalizer
    if (mlpcfg.treble || mlpcfg.bass)
    {
        Music_Emu::equalizer_t eq;

        // bass - logarithmic, 2 to 8194 Hz
        double bass = 1.0 - (mlpcfg.bass / 200.0 + 0.5);
        eq.bass = (long) (2.0 + pow( 2.0, bass * 13 ));

        // treble - -50 to 0 to +5 dB
        double treble = mlpcfg.treble / 100.0;
    ...
    {
        if (fh.m_type == gme_spc_type && mlpcfg.ignore_spc_length)
            info.length = -1;

...
static void cd_play (void) {mlp_drct_pl_open ("cdda://"); }
static void cd_add (void) {mlp_drct_pl_add ("cdda://", -1); }
...
static gboolean cd_init (void)
{
    for (gint m = 0; m < N_MENUS; m ++)
        for (gint i = 0; i < N_ITEMS; i ++)
            mlp_plugin_menu_add (menus[m], funcs[i], _(titles[i]),
             GTK_STOCK_CDROM);
...
void cd_cleanup (void)
{
    for (gint m = 0; m < N_MENUS; m ++)
        for (gint i = 0; i < N_ITEMS; i ++)
            mlp_plugin_menu_remove (menus[m], funcs[i]);
}

...
aosd_trigger_func_pb_titlechange_cb ( gpointer plentry_gp , gpointer prevs_gp )
{
  if (mlp_drct_get_playing ())
  {
    ...
    gint pl_entry = mlp_playlist_get_position(playlist);
    gchar * pl_entry_filename = mlp_playlist_entry_get_filename (playlist, pl_entry);
    gchar * pl_entry_title = mlp_playlist_entry_get_title (playlist, pl_entry, FALSE);
...
{
  gint active = mlp_playlist_get_active();
  gint pos = mlp_playlist_get_position(active);
  gchar *utf8_title, *utf8_title_markup;
  gint time_cur, time_tot;
  gint time_cur_m, time_cur_s, time_tot_m, time_tot_s;

  time_tot = mlp_playlist_entry_get_length (active, pos, FALSE) / 1000;
  time_cur = mlp_drct_get_time() / 1000;
  time_cur_s = time_cur % 60;
  time_cur_m = (time_cur - time_cur_s) / 60;
  time_tot_s = time_tot % 60;
  time_tot_m = (time_tot - time_tot_s) / 60;

  utf8_title = mlp_playlist_entry_get_title (active, pos, FALSE);

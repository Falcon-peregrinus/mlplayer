...
aosd_cb_configure_test ( gpointer cfg_win )
{
  ...
  markup_message = g_markup_printf_escaped(
    _("<span font_desc='%s'>MLPlayer OSD</span>") , cfg->osd->text.fonts_name[0] );
...
aosd_ui_configure ( aosd_cfg_t * cfg )
{
  ...
  gtk_window_set_title( GTK_WINDOW(cfg_win) , _("MLPlayer OSD - configuration") );
...
aosd_ui_about ( void )
{
  ...
  gtk_window_set_title( GTK_WINDOW(about_win), _("MLPlayer OSD - about") );
  ...
  info_tb_content = g_strjoin( NULL , _("\nMLPlayer OSD ") , AOSD_VERSION_PLUGIN ,
                               _("\nhttp://www.develia.org/projects.php?p=mlplayer#aosd\n"

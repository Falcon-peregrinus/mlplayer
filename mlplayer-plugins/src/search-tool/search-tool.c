...
static void find_playlist (void)
{
    ...
    for (gint p = 0; playlist_id < 0 && p < mlp_playlist_count (); p ++)
    {
        gchar * title = mlp_playlist_get_title (p);

        if (! strcmp (title, _("Library")))
            playlist_id = mlp_playlist_get_unique_id (p);
...
{
    gint list = mlp_playlist_get_blank ();
    mlp_playlist_set_title (list, _("Library"));
    mlp_playlist_set_active (list);
    playlist_id = mlp_playlist_get_unique_id (list);
    return list;
}
...
static gint get_playlist (gboolean require_added, gboolean require_scanned)
{
    ...
    gint list = mlp_playlist_by_unique_id (playlist_id);
    ...
    if (require_added && mlp_playlist_add_in_progress (list))
        return -1;
    if (require_scanned && mlp_playlist_scan_in_progress (list))
        return -1;
...
{
    gchar * path = mlp_get_string ("search-tool", "path");
...
static void create_dicts (gint list)
{
    ...
    gint entries = mlp_playlist_entry_count (list);

    for (gint e = 0; e < entries; e ++)
    {
        gchar * fields[FIELDS];

        Tuple * tuple = mlp_playlist_entry_get_tuple (list, e, TRUE);
        fields[GENRE] = tuple ? tuple_get_str (tuple, FIELD_GENRE, NULL) : NULL;
        if (tuple)
            tuple_unref (tuple);

        mlp_playlist_entry_describe (list, e, & fields[TITLE], & fields[ARTIST],
         & fields[ALBUM], TRUE);
...
static void begin_add (const gchar * path)
{
    ...
    mlp_set_string ("search-tool", "path", path);
    ...
    {
        gchar * filename = mlp_playlist_entry_get_filename (list, entry);

        if (g_str_has_prefix (filename, prefix) && ! g_hash_table_lookup_extended
         (added_table, filename, NULL, NULL))
        {
            mlp_playlist_entry_set_selected (list, entry, FALSE);
            g_hash_table_insert (added_table, filename, NULL);
        }
        else
        {
            mlp_playlist_entry_set_selected (list, entry, TRUE);
            str_unref (filename);
        }
    }

    mlp_playlist_delete_selected (list);
    mlp_playlist_remove_failed (list);

    Index * add = index_new ();
    index_append (add, str_get (uri));
    mlp_playlist_entry_insert_filtered (list, -1, add, NULL, filter_cb, NULL, FALSE);
...
static gint search_timeout (void * unused)
{
    ...
    if (results_list)
    {
        mlpgui_list_delete_rows (results_list, 0, mlpgui_list_row_count (results_list));
        mlpgui_list_insert_rows (results_list, 0, index_count (items));
    }
...
static void add_complete_cb (void * unused, void * unused2)
{
    if (adding)
    {
        ...
        if (list >= 0 && ! mlp_playlist_add_in_progress (list))
        {
            adding = FALSE;
            destroy_added_table ();
            mlp_playlist_sort_by_scheme (list, PLAYLIST_SORT_PATH);
        }
    }

    if (! dicts_valid && ! mlp_playlist_update_pending ())
        update_dicts ();
}
...
static void playlist_update_cb (void * data, void * unused)
{
    ...
    else
    {
        ...
        if (list < 0 || mlp_playlist_updated_range (list, & at, & count) >=
...
static void do_add (gboolean play, gchar * * title)
{
    ...
    for (gint i = 0; i < n_items; i ++)
    {
        ...
        for (gint m = 0; m < item->matches->len; m ++)
        {
            ...
            index_append (filenames, mlp_playlist_entry_get_filename (list, entry));
            index_append (tuples, mlp_playlist_entry_get_tuple (list, entry, TRUE));
        }
    ...
    mlp_playlist_entry_insert_batch (mlp_playlist_get_active (), -1, filenames,
     tuples, play);
}

static void action_play (void)
{
    gint list = mlp_playlist_get_temporary ();
    mlp_playlist_set_active (list);

    if (mlp_get_bool (NULL, "clear_playlist"))
        mlp_playlist_entry_delete (list, 0, mlp_playlist_entry_count (list));
    else
        mlp_playlist_queue_delete (list, 0, mlp_playlist_queue_count (list));
...
static void action_create_playlist (void)
{
    ...
    if (title)
        mlp_playlist_set_title (mlp_playlist_count () - 1, title);
}

static void action_add_to_playlist (void)
{
    if (mlp_playlist_by_unique_id (playlist_id) == mlp_playlist_get_active ())
        return;
...
static void * search_get_widget (void)
{
    ...
    help_label = gtk_label_new (_("To import your music library into "
     "MLPlayer, choose a folder and then click the \"refresh\" icon."));
    ...
    results_list = mlpgui_list_new (& list_callbacks, NULL, items ? index_count (items) : 0);
    ...
    mlpgui_list_add_column (results_list, NULL, 0, G_TYPE_STRING, -1);

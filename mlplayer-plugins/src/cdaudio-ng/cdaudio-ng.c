/*
 * MLPlayer CD Digital Audio plugin
 *
...
{
    gint length = mlp_playlist_entry_count (playlist);

    for (gint count = 0; count < length; count ++)
    {
        gchar * filename = mlp_playlist_entry_get_filename (playlist, count);

        if (cdaudio_is_our_file (filename, NULL))
        {
            mlp_playlist_entry_delete (playlist, count, 1);
            count--;
            length--;
        }

...
void alarm_save(GtkButton *w, gpointer data)
{
   ...
   for(; daynum < 7; daynum++)
   {
     ...
     mlp_set_int ("alarm", day_flags[daynum], alarm_conf.day[daynum].flags);
     mlp_set_int ("alarm", day_h[daynum], alarm_conf.day[daynum].hour);
     mlp_set_int ("alarm", day_m[daynum], alarm_conf.day[daynum].min);
   }

   /* END: days of week */

   volume = gtk_range_get_value (alarm_conf.volume);
   mlp_set_int ("alarm", "volume", volume);

   quietvol = gtk_range_get_value (alarm_conf.quietvol);
   mlp_set_int ("alarm", "quietvol", quietvol);

   fading = gtk_spin_button_get_value_as_int (alarm_conf.fading);

   /* write the new values */
   mlp_set_int ("alarm", "stop_h", stop_h);
   mlp_set_int ("alarm", "stop_m", stop_m);
   mlp_set_int ("alarm", "fading", fading);
   mlp_set_bool ("alarm", "stop_on", stop_on);

   g_free (cmdstr);
   cmdstr = gtk_editable_get_chars ((GtkEditable *) alarm_conf.cmdstr, 0, -1);
   mlp_set_string ("alarm", "cmdstr", cmdstr);

   cmd_on = gtk_toggle_button_get_active (alarm_conf.cmd_on);
   mlp_set_bool ("alarm", "cmd_on", cmd_on);

   g_free (playlist);
   playlist = gtk_editable_get_chars ((GtkEditable *) alarm_conf.playlist, 0, -1);
   mlp_set_string ("alarm", "playlist", playlist);

   /* reminder */
   g_free (alarm_conf.reminder_msg);
   alarm_conf.reminder_msg = gtk_editable_get_chars ((GtkEditable *) alarm_conf.reminder, 0, -1);
   mlp_set_string ("alarm", "reminder_msg", alarm_conf.reminder_msg);

   alarm_conf.reminder_on = gtk_toggle_button_get_active (alarm_conf.reminder_cb);
   mlp_set_bool ("alarm", "reminder_on", alarm_conf.reminder_on);
}
...
static void alarm_read_config(void)
{
   ...
   mlp_config_set_defaults ("alarm", alarm_defaults);

   alarm_h = mlp_get_int ("alarm", "alarm_h");
   alarm_m = mlp_get_int ("alarm", "alarm_m");

   /* save them here too */
   alarm_conf.default_hour = alarm_h;
   alarm_conf.default_min = alarm_m;

   stop_h = mlp_get_int ("alarm", "stop_h");
   stop_m = mlp_get_int ("alarm", "stop_m");
   stop_on = mlp_get_bool ("alarm", "stop_on");

   volume = mlp_get_int ("alarm", "volume");
   quietvol = mlp_get_int ("alarm", "quietvol");

   fading = mlp_get_int ("alarm", "fading");

   g_free (cmdstr);
   cmdstr = mlp_get_string ("alarm", "cmdstr");
   cmd_on = mlp_get_bool ("alarm", "cmd_on");

   g_free (playlist);
   playlist = mlp_get_string ("alarm", "playlist");

   g_free (alarm_conf.reminder_msg);
   alarm_conf.reminder_msg = mlp_get_string ("alarm", "reminder_msg");
   alarm_conf.reminder_on = mlp_get_bool ("alarm", "reminder_on");

   /* day flags and times */
   for(; daynum < 7; daynum++)
   {
     /* read the flags */
     alarm_conf.day[daynum].flags = mlp_get_int ("alarm", day_flags[daynum]);

     /* read the times */
     alarm_conf.day[daynum].hour = mlp_get_int ("alarm", day_h[daynum]);
     alarm_conf.day[daynum].min = mlp_get_int ("alarm", day_m[daynum]);
   }
}
...
static void alarm_about(void)
{
   ...
   mlpgui_simple_message (& about_dialog, GTK_MESSAGE_INFO, _("About Alarm"),
...
static void *alarm_fade(void *arg)
{
    ...
    */
   //mlp_drct_set_volume_main((gint)vols->end);
...
static void *alarm_stop_thread( void *args )
{
   ...
   mlp_drct_get_volume_main(&currvol),
    ...
    */
   mlp_drct_set_volume_main(currvol);
...
static gboolean alarm_timeout (void * unused)
{
   ...
   guint today;
     ...
     if(fading)
     {
       ...
       mlp_drct_set_volume_main(quietvol);
     ...
     else
     {
       ...
       /* set volume */
       mlp_drct_set_volume_main(volume);
